class AssignatureAbility < ActiveRecord::Base
  ...
  def merge_into!
    other = AssignatureAbility.where('lower(nombre) = ? AND assignature_id = ? ', self.nombre.try(:mb_chars).try(:downcase).try(:to_s), self.assignature.id).where.not(id: self.id).first
    #other = Content.where(nombre: self.nombre).where(assignature_id: self.assignature_id).first
    if other.present?
      other.questions.update_all(assignature_ability_id: self.id)
      other.destroy
    end
  end
end
